...
| **OFAC SDN + Consolidated Non-SDN** | `NOT_YET_IMPLEMENTED` | **Why it's hard / Planned**: Treasury OFAC publishes ~17,000+ entries across XML (`sdn.xml`) and 3 relational CSVs (`sdn.csv`, `add.csv`, `alt.csv`). Synchronous HTTP fetching risks Vercel serverless function timeout (60s limit). Requires a durable background Inngest worker with streaming XML/CSV parsing and chunked DB transactions to ensure 100% full-list completeness without truncation. |
