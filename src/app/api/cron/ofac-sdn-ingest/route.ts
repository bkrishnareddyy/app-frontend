...
async function handleIngest(req: Request, requestId: string) {
  ...
  try {
    const result = await ScreeningIngestionService.fetchAndIngestOfacSdn();
    ...
    console.error("[ofac-sdn-ingest] Failed:", err);
    return NextResponse.json(
      { status: "FAILED", requestId, error: err.message || "OFAC SDN ingestion failed" },
      { status: 500 }
    );
  }
}
