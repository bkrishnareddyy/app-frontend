...
async function handleIngest(req: Request, requestId: string) {
  ...
  try {
    const result = await AdCvdIngestionService.fetchAndIngestAdCvdCompanyRates();
    ...
    console.error("[ad-cvd-company-rates-ingest] Failed:", err);
    return NextResponse.json(
      { status: "FAILED", requestId, error: err.message || "AD/CVD company rates ingestion failed" },
      { status: 500 }
    );
  }
}
