...
async function handleIngest(req: Request, requestId: string) {
  ...
  try {
    const result = await TradeAgreementIngestionService.fetchAndIngestCaftaDrRules();
    ...
    console.error("[cafta-dr-rules-ingest] Failed:", err);
    return NextResponse.json(
      { status: "FAILED", requestId, error: err.message || "CAFTA-DR rules ingestion failed" },
      { status: 500 }
    );
  }
}
