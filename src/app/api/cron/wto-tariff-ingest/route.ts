...
async function handleIngest(req: Request, requestId: string) {
  ...
  try {
    const result = await TradeDataIngestionService.fetchAndIngestWtoTariffs();
    ...
    console.error("[wto-tariff-ingest] Failed:", err);
    return NextResponse.json(
      { status: "FAILED", requestId, error: err.message || "WTO tariff ingestion failed" },
      { status: 500 }
    );
  }
}
