...
async function handleIngest(req: Request, requestId: string) {
  ...
  try {
    const result = await TariffRemedyIngestionService.fetchAndIngestSection301Rates();
    ...
    console.error("[section-301-rates-ingest] Failed:", err);
    return NextResponse.json(
      { status: "FAILED", requestId, error: err.message || "Section 301 rates ingestion failed" },
      { status: 500 }
    );
  }
}
