...
async function handleIngest(req: Request, requestId: string) {
  ...
  try {
    const result = await TariffRemedyIngestionService.fetchAndIngestSection301Exclusions();
    ...
    console.error("[section-301-exclusions-ingest] Failed:", err);
    return NextResponse.json(
      { status: "FAILED", requestId, error: err.message || "Section 301 exclusions ingestion failed" },
      { status: 500 }
    );
  }
}
