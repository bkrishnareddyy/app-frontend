...
export default async function AdminSettingsPage() {
  ...
  if (!canAccessHref(context, "/app/admin/settings")) {
    redirect("/app/work");
  }
              ...
              <div key={log.id} className="p-4 hover:bg-slate-50 transition-colors flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    <span className="px-2 py-0.5 rounded-full text-sm font-mono font-bold bg-blue-50 text-[#0071E3] border border-blue-100">
                      {log.action}
                    </span>
                  ...
                  {log.metadata && (
                    <pre className="text-sm font-mono text-[#1D1D1F] bg-[#F5F5F7] p-2.5 rounded-xl border border-[#E5E5EA] overflow-x-auto max-w-xl">
                      {JSON.stringify(log.metadata, null, 2)}
                    </pre>
                  )}
                </div>
                <div className="text-right text-sm text-[#86868B] whitespace-nowrap">
