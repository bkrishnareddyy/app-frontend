...
interface CommandCenterClientProps {
  ...
  atRiskCount: number;
  avgReadiness: number;
  reviewRequiredDecisions: number;
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-[#1D1D1F]">
                ...
                <tbody className="divide-y divide-[#E5E5EA]">
                  {shipments.slice(0, 6).map((shp: any) => (
                    <tr key={shp.id} className="hover:bg-[#F5F5F7]/50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-[#86868B]">{shp.exporterName || shp.shipper || "Shenzhen Hardware Corp"}</td>
                      <td className="py-3 px-4 font-mono text-[11px] text-[#1D1D1F]">{shp.primaryHtsCode || "7318.15.2065"}</td>
                      <td className="py-3 px-4 font-semibold">${(shp.totalValue || 48500.0).toLocaleString()}</td>
                      <td className="py-3 px-4 font-bold text-emerald-600">{shp.readinessScore || 96}%</td>
