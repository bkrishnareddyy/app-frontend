...
function PanelError({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center h-full text-center max-w-sm mx-auto space-y-3">
      ...
      <p className="text-sm text-[#1D1D1F] font-semibold">Couldn&apos;t load this section</p>
      <p className="text-xs text-[#86868B]">{message}</p>
      <button
        onClick={onRetry}
        className="px-4 py-2 text-xs font-semibold text-[#0071E3] bg-blue-50 hover:bg-blue-100 rounded-full transition-colors cursor-pointer"
      >
        Try again
      </button>
    </div>
...
export function ManageAccountModal({ isOpen, onClose, accountName, items, externalItems = [] }: ManageAccountModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl border border-[#E5E5EA] shadow-2xl w-full max-w-5xl h-[85vh] overflow-hidden flex text-sm">
        <button
          onClick={onClose}
        ...
        {/* Left nav rail */}
        <div className="w-52 shrink-0 bg-[#FAFAFA] border-r border-[#E5E5EA] p-3 flex flex-col overflow-y-auto">
          <p className="px-2 text-[10px] font-bold uppercase tracking-wider text-[#86868B] mb-2 truncate" title={accountName}>
            {accountName}
          </p>
          <nav className="space-y-0.5">
            {items.map((item, idx) => {
              ...
              return (
                ...
                >
                  <Icon className={cn("w-3.5 h-3.5 shrink-0", isSelected ? "text-[#0071E3]" : "text-[#86868B]")} />
                ...
                {externalItems.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      onClick={onClose}
                      title={item.description}
                      className="w-full flex items-center space-x-2 px-2.5 py-1.5 rounded-lg text-left text-[13px] font-medium text-[#1D1D1F] hover:bg-white/60 transition-all cursor-pointer"
                    >
                      <Icon className="w-3.5 h-3.5 shrink-0 text-[#86868B]" />
                      <span className="truncate flex-1">{item.name}</span>
                      <ExternalLink className="w-3 h-3 shrink-0 text-[#86868B]" />
                    </Link>
                  );
                })}
              </nav>
